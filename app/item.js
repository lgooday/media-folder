...
export default class Item {
	...
	process() {
		return this.init()
			.then(() => {
				return this.checkIfExistsInDb()
			})
			.then(() => {
				return this.prepare()
			})
			.then(() => {
				return this.copyToDest()
			})
			.then(() => {
	...
	checkIfExistsInDb() {
		return db.queryOne('SELECT COUNT(hash) AS alreadyExists FROM item WHERE hash = ?', [this.hash], 'checkIfExistsInDb')
			.then(
				row => {
					this.alreadyExists = row.alreadyExists !== 0
				}
			)
	}

	insertInDb() {
		return db.queryAll('INSERT IGNORE INTO item SET ?', this.toDb(), 'insertInDb')
	}
			...
			.then(
				pRes => {
					...
					if (!this.lastModif &&
						...
						const dt = moment(pRes[1].exif.exif.DateTimeOriginal, 'YYYY:MM:DD HH:mm:ss', true)
	...
	tryFilename() {
		return new Promise(res => {
			...
			if (filename.length === 15) {
				const dt = moment(filename, 'YYYYMMDD_HHmmss')

				if (dt.isValid()) {
					res({ verdict: true, dt })
				} else {
					res({ verdict: false, reason: 'invalid' })
				}
			} else {
				res({ verdict: false, reason: 'len' })
			}
		})
	}
			...
			try {
				fs.stat(this.src, (err, stats) => {
					...
					if (m.isValid()) {
						...
						rej({ type: 'tryFs', err: stats.mtime })
					}
				})
			} catch (err) {
				rej({ type: 'tryFs', err })
	...
	tryExif() {
		return new Promise(res => {
			if (exifAllowedType.indexOf(this.ext) >= 0) {
				try {
					new exif.ExifImage({ image: this.src }, (err, exif) => { // eslint-disable-line  no-new
						if (err) {
							res({ verdict: false, err })
						} else {
							res({ verdict: true, exif })
						}
					})
				} catch (err) {
					res({ verdict: false, err })
	...
	moveToBackup() {
		return fs.move(this.src, pathjoin(this.absBackupPath, this.baseName), { overwrite: true })
	}
}
